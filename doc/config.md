...
Чтобы подключить «Конструктор» к вашему модулю, необходимо:
* в папке `lib` вашего модуля создать файл с классом настроек и унаследовать его от `\Rover\Fadmin\Options`. Этот класс должен реализовывать публичный метод `getConfig()`. В решении есть демо-файл `lib/testoptions.php`, в котором находится пример такого класса. Страницу настроек, которая получается на основе конфигурации, возвращаемой `getConfig()` этого класса, вы можете найти в административной части сайта в разделе «Настройки» -> «Настройки продукта» -> «Настройки модулей» -> «Констуктор администативной части».
* в файле `options.php` вашего модуля запустить «Конструктор административной части». Примерную структуру этого файла можно взять из файла `options.php` «Конструктора».
...
`getConfig()` возвращает ассоциативный массив с 2мя ключами. В `'tabs'` находится массивы с конфигурацией вкладок и полей страницы настроек модуля, в `'settings'` – другие общие настройки. Если ключ `tabs` пуст, то табы с настройками модуля не выводятся. Если ключ `settings` пуст, то берутся настройки по умолчанию. 
###Структура `'settings'`
