...
##Сообщения
Для информирования администратора о непредвиденных ситуациях, либо об успешном выполнении операции, в вашем классе, унаследованном от `\Rover\Fadmin\Options` доступен метод `public addMessage($message, $type = 'OK')`. Параметр `$type` может принимать значения `'OK'` и `'ERROR'`.
