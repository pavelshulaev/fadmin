...
##Описание
«Конструктор административной части» - это фреймворк,  позволяющий быстро настроить и запустить административную часть для модуля Битрикс. Отрисовку и работу с опциями административной части фреймворк берет на себя. 

Для подключения и использования фреймворка программисту необходимо в своём модуле создать файл, унаследованный от `\Rover\Fadmin\Options` и переопределить в нем метод `getConfig()`. Из этого метода следует вернуть конфигурацию для «Конструктора». Конфигурация состоит из описания вкладок и полей на них и прочих параметров.
...
##События
Для возможности более гибкой работы с настройками в «Конструкторе» предусмотрена система событий. Название всех событий определены в классе `\Rover\Fadmin\Options` в константах, начинающихся на `EVENT__`. Подробнее о событиях вы можете почитать в [api по \Rover\Fadmin\Options](./doc/options.md#Константы)
...
Методы добавляют сообщения об успешном и неуспешном выполнении операции соответственно. Сообщения затем выводятся над вкладками настроек вашего модуля.
##API
###[Rover\Fadmin\Options](./doc/options.md)
##Требования
