...
##Описание
«Быстрая админка» - это фреймворк,  позволяющий быстро настроить и запустить административную часть для модуля Битрикс. Отрисовку административной части и работу с опциями фреймворк берет на себя. Вам необходимо будет создать файл, унаследованный от `\Rover\Fadmin\Options`, в котором следует указать расположение полей и дать им уникальные имена. Фреймворк поддерживает расположение элементов на вкладках, создание пресетов и отдельные настройки для каждого сайта.
##Использование
Чтобы подключить фреймворк к вашему модулю, необходимо создать 2 файла.
###Файл `options.php`
Файл `options.php` входит в стандартный набор файлов любого модуля, в нём обычно находится логика, отвечающая за административную часть и работу с её настройками. Для подключения «Быстрой админки» этот файл должен иметь подобное содержание:
...
В самом фреймворке есть демо-файл `lib/testoptions.php`, в котором находится пример такого класса. Страницу настроек, которая получается на основе настроек в этом файле, вы можете найти в административной части сайта в разделе «Настройки» -> «Настройки продукта» -> «Настройки модулей» -> «Быстрая админка».
...
Со всеми параметрами вы можете ознакомиться, изучив классы инпутов в «Быстрой админке».
...
##События
Для возможности более гибкой в фреймворке предусмотрена система событий. Название всех событий определены в классе `\Rover\Fadmin\Options` в константах, начинающихся на `EVENT__`.
